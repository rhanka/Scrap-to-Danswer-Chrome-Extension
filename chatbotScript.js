(function() {
    ...
    function createChatSession() {
        chrome.storage.sync.get(['danswerHost', 'danswerToken','danswerAssistantId'], (data) => {
            ...
            .then(data => {
                ...
                chrome.runtime.sendMessage({ action: 'setChatSessionId', chatSessionId: chatSessionId });
    ...
    function restoreChatSession() {
        chrome.storage.sync.get(['danswerHost', 'danswerToken'], (data) => {
            ...
            fetch(`${host}/api/chat/get-chat-session/${chatSessionId}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': token
            ...
            .then(data => {
                data.messages.forEach((m) => {
                    displayMessage(m.message,m.message_type)
                })
            })
            .catch(error => {
    ...
    function sendMessage() {
      chrome.storage.sync.get(['danswerHost', 'danswerToken', 'danswerAssistantId'], (data) => {
        const { danswerHost: host, danswerToken: token, danswerAssistantId: assistantId } = data;
        const userInput = document.getElementById('userInput').value;
        if (userInput.trim() === '') return;
 
        if (!host || !token || assistantId == undefined) {
            console.error("Host, token not configured");
            displayMessage('Danswer Host, token not configured.', 'assistant');
            return;
        }
        ...
        // Afficher la bulle d'attente
        displayWaitingBubble('Interrogation de l\'assistant');


        blockJsonParseFail = new RegExp('"answer_piece"');
        // Envoyer le message à l'agent Danswer
        fetch(`${host}/api/chat/send-message`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': token
        ...
        .then(response => {
            ...
            return reader.read().then(function processText({ done, value }) {
                if (done) {
                    if (buffer) {
                        processJson(buffer); // Traiter le dernier morceau
                    }
                ...
                while ((end = buffer.indexOf('\n', start)) !== -1) {
                    ...
                    if (chunk) {
                        processJson(chunk); // Traiter chaque morceau JSON
                    }
                    start = end + 1;
                }
        ...
        //.then(data => {
        //    // Masquer la bulle d'attente
        //    hideWaitingBubble();
        //    // Afficher la réponse de l'agent Danswer
    ...
    function processJson(jsonRaw) {
        if (jsonRaw) {
            try {
                let json = JSON.parse(jsonRaw)
                if (json.reserved_assistant_message_id) {
                    parentMessageId = json.reserved_assistant_message_id
                } else if (json.tool_name) {
                    displayWaitingBubble(`${json.tool_name}: ${json.tool_args && json.tool_args.query}`)
                } else if (json.top_documents) {
                    refDocuments = json.top_documents;
                } else if (json.answer_piece) {
                    hideWaitingBubble();
                    currentMarkdown = currentMarkdown + json.answer_piece;
                    displayMessage(currentMarkdown, 'assistant');
                }
            } catch(e) {
    ...
    function displayMessage(message, sender) {
      ...
      const messageElement = currentMessageElement || document.createElement('div');
    ...
    function displayWaitingBubble(waitingMessage) {
        const chatbox = document.getElementById('chatbox');
        const waitingBubble = document.createElement('div');
        waitingBubble.id = 'waitingBubble';
        waitingBubble.className = 'message bot wait';
        waitingBubble.textContent = waitingMessage+'.';
        chatbox.appendChild(waitingBubble);
        chatbox.scrollTop = chatbox.scrollHeight;
      
        let dots = 1;
        waitingInterval = setInterval(() => {
          waitingBubble.textContent = waitingMessage+'.'.repeat(dots);
          dots = (dots % 3) + 1;
        }, 500);
      }
